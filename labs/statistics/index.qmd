...
The prostate cancer data set comes from The Elements of Statistical Learning book by Trevor Hastie, Robert Tibshirani and Jerome Friedman. The data contains measures of prostate specific antigen (PSA) and a number of clinical measures in men who were about to receive a radical prostatectomy.
```{r}
## Download the dataset
...
$$
\hat{\beta}=\frac{1}{2}\min\limits_{\beta}\{\sum_{i=1}^N
...
From the lasso fit, Rating was selected and had a beta coefficient of `{r} round(coef(lassoFit, s = "lambda.1se")["Rating", ], 4)`. Compare to that of the Limit predictor. What do you notice about the correlation between Ratings and Limit to each other and to the response?
...
::: {.callout-note appearance="simple" icon=false}
## Task
Apply the stability selection method of choice (lasso or randomized) on the prostate cancer dataset. Decide on the parameter values and play with them. Examine the output of `randLassoStabSel()` and plot the stability paths. What are you selecting?
:::

## Bonus exercise

Use the prostate cancer data set apply a ridge regression, a lasso and lasso stability selection. Compare the coefficients you are getting and what gets selected, keeping in mind what the correlation structure looks like as well in the predictor matrix.
...
Additional reading material and interesting resources:
* Meinshausen paper
* blogs wxw

end with honorable mentions like logistic regression, multivariate regression, graphical lasso, GGMs and the beautiful properties there and how lasso is used to estimate the graph -> save it for the slides
