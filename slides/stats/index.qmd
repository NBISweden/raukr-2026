...
::: {.fragment}
- Answer: Definitely not. Why: Our aim is to use the model on new data and not to explain the data we already have. 
- Overfitting -- Be skeptical when someone claims 100% accuracy with prediction ..
...
::: {.aside}
[Image from OSCA](https://bioconductor.org/books/3.23/OSCA.basic/index.html)
:::
...
- Overall questions on the linear models and concepts we have explored so far before we move to some interesting uses and applications?

## Break here with an example data set and ask students which method they would use?
...
df$fitted <- fit1$fitted
ggplot(df, aes(x = x, y = y)) + 
    geom_point() + 
    geom_point(aes(x = x, y = fitted), color = "firebrick") + 
    theme_bw()
...
df$fittedSpanPt9 <- fit2$fitted
ggplot(df, aes(x = x, y = y)) + 
    geom_point() + 
    geom_point(aes(x = x, y = fittedSpanPt9), color = "firebrick") + 
    theme_bw()
...
df$fittedSpanPt5 <- fit3$fitted
ggplot(df, aes(x = x, y = y)) + 
    geom_point() + 
    geom_point(aes(x = x, y = fittedSpanPt5), color = "firebrick") + 
    theme_bw()

```

## `Limma`'s normalize cyclic loess and example of GC bias
...
![](assets/GCbias1.webp){width=60%}

![](assets/GCbias2.webp){width=60%}
